...
# scan files
log_msg("Running Chazam fingerprinter on files in %s..." % test_folder, 
        log=options.log, silent=options.silent)

tm = time.time()
cha = ChazamTest(options.temp_folder, test_seconds)
log_msg("finished obtaining results from chazam in %s" % (time.time() - tm),
        log=options.log, silent=options.silent)
...
for line in range(0, cha.n_lines):
	for col in range(0, cha.n_columns):
		# for chazam
		all_query_duration[col][line][0] = cha.result_query_duration[line][col]
		all_match_confidence[col][line][0] = cha.result_match_confidence[line][col]

		cha_match_result = cha.result_match[line][col]

		if cha_match_result == 'yes':
			all_match_counter[col][0][0] += 1
		elif cha_match_result == 'no':
			all_match_counter[col][1][0] += 1
		else:
			all_match_counter[col][2][0] += 1

		cha_match_acc = cha.result_matching_times[line][col]

		if cha_match_acc == 0 and cha_match_result == 'yes':
			all_matching_times_counter[col][0][0] += 1
		elif cha_match_acc != 0:
			all_matching_times_counter[col][1][0] += 1

# create plots
cha.create_plots('Confidence', all_match_confidence, options.results_folder)
cha.create_plots('Query duration', all_query_duration, options.results_folder)
...
for sec in range(0, n_secs):
    ...
    means_cha = [round(x[0] * 100 / cha.n_lines, 1) for x in all_match_counter[sec]]
    rects1 = ax.bar(ind, means_cha, width, color='r')
    ...
    #ax.legend((rects1[0]), ('Chazam'), loc='center left', bbox_to_anchor=(1, 0.5))
...
for sec in range(0, n_secs):
    ...
    means_cha = [round(x[0] * 100 / div, 1) for x in all_matching_times_counter[sec]]
    rects1 = ax.bar(ind, means_cha, width, color='r')

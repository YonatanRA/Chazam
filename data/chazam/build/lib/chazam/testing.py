...
from chazam.decoder import path_to_songname
from chazam import Chazam
from chazam.fingerprint import *
import traceback
import fnmatch
import os, re, ast
import subprocess
...
class ChazamTest(object):
    def __init__(self, folder, seconds):
        super(ChazamTest, self).__init__()
        ...
        for f in self.test_files:
            ...
            if result.strip() == "None":
                ...
                if song_result != song:
                    ...
                    self.result_query_duration[line][col] = round(result[Chazam.MATCH_TIME],3)
                    self.result_match_confidence[line][col] = result[Chazam.CONFIDENCE]

                    song_start_time = re.findall("\_[^\_]+",f)
                    song_start_time = song_start_time[0].lstrip("_ ")

                    result_start_time = round((result[Chazam.OFFSET] * DEFAULT_WINDOW_SIZE * 
                    ...
                    log_msg('query duration: %s' % round(result[Chazam.MATCH_TIME],3))
                    log_msg('confidence: %s' % result[Chazam.CONFIDENCE])
